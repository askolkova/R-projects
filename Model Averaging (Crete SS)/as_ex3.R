...
## EX 3
#### Stein Shrinkage towards Restrictions
# Going to shrink the model 7 to the model 3
theta_unconstr <- b7
theta_constr = as.matrix(c(b3[1:8], matrix(0, 1, 4), b7[9:13]))
# x3 <- as.matrix(cbind(edu12, edu13, edu14, edu16, edu18, edu20, exp, exp2, married, ne, mw, south, matrix(1, n, 1)))
# x7 <- as.matrix(cbind(edu12, edu13, edu14, edu16, edu18, edu20, exp, exp2, exp3, exp4, exp5, exp6, married, ne, mw, south, matrix(1, n, 1)))
diff <- theta_unconstr - theta_constr 
V_inv <- solve(var_mat7)
theta_shrunk <- theta_unconstr -  diff * ((6-2)/ (t(diff) %*% V_inv %*% diff))[1,1] 
